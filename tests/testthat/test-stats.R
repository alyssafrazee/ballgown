...
test_that('default stat models work', {
    expect_that(stattest(bg, feature='transcript', meas='FPKM', covariate='group'), 
        not(throws_error()))
    expect_that(stattest(bg, feature='transcript', meas='cov', covariate='group'), 
        not(throws_error()))
    expect_that(stattest(bg, feature='transcript', meas='ucount', covariate='group'), 
        throws_error())
    ...
    expect_that(stattest(bg, feature='intron', meas='mrcount', covariate='group'), 
        not(throws_error()))
    expect_that(stattest(bg, feature='intron', meas='cov', covariate='group'), throws_error())

    # check other errors:
    expect_that(stattest(bg, feature='junction', meas='mrcount', covariate='group'), throws_error())
    expect_that(stattest(bg, feature='transcript', meas='FPKM', covariate=group), throws_error())
    expect_that(stattest(bg, feature='transcript', meas='FPKM', covariate='nonsense!'), 
        throws_error())
    bgnull = bg
    pData(bgnull) = NULL
    expect_that(stattest(bgnull, feature='transcript', meas='FPKM', covariate='group'), 
        throws_error())
    rm(bgnull)
})

test_that('log, library size, and getFC options work', {
    expect_that(stattest(bg, feature='transcript', meas='FPKM', covariate='group', log=FALSE), 
        not(throws_error()))
    expect_that(stattest(bg, feature='transcript', meas='FPKM', covariate='group', libadjust=FALSE),
        not(throws_error()))
    expect_that(stattest(bg, feature='transcript', meas='FPKM', covariate='group', getFC=TRUE), 
        not(throws_error()))
    expect_that(stattest(bg, feature='transcript', meas='FPKM', covariate='group', log=FALSE, 
        getFC=TRUE), gives_warning())
})
...
test_that('user-defined model matrices work', {
    ...
    expect_that(stattest(bg, mod=mod, mod0=mod0, feature='transcript', meas='FPKM'), 
        not(throws_error()))
    customres = stattest(bg, mod=mod, mod0=mod0, feature='transcript', meas='FPKM')
    ...
    expect_that(stattest(bg, mod=mod, mod0=mod0, feature='transcript', meas='FPKM', getFC=TRUE), 
        gives_warning())
    expect_that(stattest(bg, mod=mod0, mod0=mod, feature='transcript', meas='FPKM'),
        throws_error())
    m0 = model.matrix(~pData(bg)$c1 + pData(bg)$c2)
    m = model.matrix(~pData(bg)$c2 + pData(bg)$group + rnorm(20)) #non-nested
    expect_that(stattest(bg, mod=m, mod0=m0, feature='transcript', meas='FPKM'), throws_error())
})

test_that('adjustment for covariates works', {
    expect_that(stattest(bg, feature='transcript', meas='FPKM', covariate='group', adjustvars='c1'),
        not(throws_error()))
    adjres = stattest(bg, feature='transcript', meas='FPKM', covariate='group', adjustvars='c1')
    ...
    expect_that(stattest(bg, feature='transcript', meas='FPKM', covariate='group', 
        adjustvars=c('c1', 'c2', 'nonsense')), throws_error())
    expect_that(stattest(bg, feature='transcript', meas='FPKM', covariate='group', 
        adjustvars='group'), throws_error())
})


test_that('timecourse option works', {
    expect_that(stattest(bg, feature='transcript', meas='FPKM', covariate='c1', timecourse=TRUE), 
        not(throws_error()))
    tres = stattest(bg, feature='transcript', meas='FPKM', covariate='c1', timecourse=TRUE)
    ...
    expect_that(stattest(bg, feature='transcript', meas='FPKM', covariate='c1'), throws_error())
    expect_that(stattest(bg, feature='transcript', meas='FPKM', covariate='c1', timecourse=TRUE, 
        df=3), not(throws_error()))
})

test_that('stattest_table functionality works', {
    tab = texpr(bg)
    orig_res = stattest(bg, feature='transcript', meas='FPKM', covariate='group')
    tab_res = stattest(gowntable=tab, pData=pData(bg), feature='transcript', covariate='group')
