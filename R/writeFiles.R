...
#'
#' @description create tablemaker-like files on disk from a given ballgown object
...
writeFiles = function(gown, dataDir){
    ...
    if(gown@meas != 'all'){
        msg = 'can only write full ballgown objects to disk, for now. (you only have specific 
            measurements in yours).'
        stop(.makepretty(msg))
    }
    ...
    for(f in sampleNames(gown)){
        ...
        write.table(indexes(gown)$i2t, file=paste0(out, '/i2t.ctab'), quote=FALSE, col.names=TRUE, 
            row.names=FALSE, sep='\t')
        write.table(indexes(gown)$e2t, file=paste0(out, '/e2t.ctab'), quote=FALSE, col.names=TRUE, 
            row.names=FALSE, sep='\t')
        ...
        write.table(e_data, file=paste0(out, '/e_data.ctab'), quote=FALSE, col.names=TRUE,
            row.names=FALSE, sep='\t')
        ...
        write.table(i_data, file=paste0(out, '/i_data.ctab'), quote=FALSE, col.names=TRUE,
            row.names=FALSE, sep='\t')
        ...
        write.table(t_data, file=paste0(out, '/t_data.ctab'), quote=FALSE, col.names=TRUE,
            row.names=FALSE, sep='\t')
