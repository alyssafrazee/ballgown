...
#' @param exonsonly if TRUE, return transcripts only as a set of exons - do not include CDS, 
#' start/stop codons, etc.
#' 
#' @return A \code{GRangesList} object with each GRangesElement denoting a transcript in the gtf 
#' file.
#' 
...
gff2grlist = function(gtf, txidentifier = "transcript_id", exonsonly = TRUE){
    ...
    gtf.grl = lapply(gtf.list, function(x){
        x$strand[x$strand=="."] = "*"
        GRanges(seqnames=x$seqname, ranges=IRanges(start=x$start, end=x$end), strand=x$strand, 
            id=rep(0, length(x$seqname)), transcripts=rep("NA", length(x$seqname)))
    })
