...
plotLatentTranscripts = function(gene, gown, method=c("hclust", "kmeans"), k=NULL, 
    ...
    if(!is.null(k)){
        ...
        if(i==ntranscripts-1 & cl$pctvar < 0.9){
            ...
            warning("k = n-1 did not explain 90% of variation. Try no clustering, or specifying k.")
    ...
    cols = suppressWarnings(brewer.pal(numcolors, 'Dark2')) #have 2-color graph w/o warning
