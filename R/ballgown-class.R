...
#'
#' @param samples vector of file paths to folders containing sample-specific ballgown data (generated by \code{tablemaker}).  If \code{samples} is provided, \code{dataDir} and \code{samplePattern} are not used.
#' @param dataDir file path to top-level directory containing sample-specific folders with ballgown data in them.  Only used if \code{samples} is NULL.
#' @param samplePattern regular expression identifying the subdirectories of \code{dataDir} containing data to be loaded into the ballgown object (and only those subdirectories).  Only used if \code{samples} is NULL.
#' @param bamfiles optional vector of file paths to read alignment files for each sample.  If provided, make sure to sort properly (e.g., in the same order as \code{samples}).  Default NULL.
...
#' @export
ballgown = function(samples=NULL, dataDir=NULL, samplePattern=NULL, bamfiles = NULL, pData = NULL, verbose=TRUE, ...) {
    ...
    if(is.null(samples)){
        if(is.null(samplePattern)|is.null(dataDir)) stop("must provide both \"dataDir\" and \"samplePattern\" if \"samples\" is NULL.")
    ...
    e2t <- read.table(list.files(samples[1], "e2t.ctab", full.names=TRUE), header=TRUE, sep="\t", colClasses=c("integer", "integer"))
    i2t <- read.table(list.files(samples[1], "i2t.ctab", full.names=TRUE), header=TRUE, sep="\t", colClasses=c("integer", "integer"))
...
#'   character strings appropriately). Note that \code{genomesubset} is \code{TRUE} by default.
