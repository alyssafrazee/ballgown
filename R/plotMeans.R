...
#'
#' @param gene name of gene whose transcripts will be plotted.  When using Cufflinks/Tablemaker
#'   output, usually of the form \code{"XLOC_######"}
...
#' @param meas type of expression measurement to plot. One of "cov", "FPKM", "rcount", "ucount", 
    ...
    for(p in 1:numplots){
        ...
        for(tx in unique(gtrans$tid)){
            ...
            for(exind in 1:dim(gtsub)[1]){
                if(colorby == "exon"){ 
                    mycolor = closestColor(emeans[which(names(emeans)==gtsub$id[exind])], colscale)
                }
