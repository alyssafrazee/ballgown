...
Users need to run the `tablemaker` binary to get their assembly output organized into a format that `ballgown` can load.  `tablemaker` is available in the `tablemaker` subfolder of this repo; download/untar/unzip to get READMEs and binaries, or you can build from source using [Cufflinks' instructions](http://cufflinks.cbcb.umd.edu/tutorial.html). This program needs to be run on each RNA-seq sample in your experiment.  It requires your merged assembly, in `gtf` format, and the read alignments for each sample, in `bam` format.  You use `tablemaker` as follows:
...
The default statistical test in ballgown is a parametric F-test comparing nested linear models.  This is described in detail in the [Ballgown manuscript](http://biorxiv.org/content/early/2014/03/30/003665), but briefly, two models are fit to each feature, using expression as the outcome: one including the covariate of interest (e.g., case/control status or time) and one not including that covariate.  An [F statistic](http://en.wikipedia.org/wiki/F-test#Regression_problems) is calculated using the fits of the two models, and a corresponding p-value is obtained.  A significant p-value means the model including the covariate of interest fits significantly better than the model without that covariate, which indicates differential expression.  We adjust for multiple testing by reporting q-values for each transcript in addition to p-values: reporting features with, say, q < 0.05 means the false discovery rate should be controlled at about 5%. 
...
Models are fit using `log2(meas + 1)` for each feature. To disable this transformation, provide `log = FALSE` as an argument to `stattest`.

If you have a feature-by-sample matrix of expression values outside of a `ballgown` object, you can run `ballgown`'s statistical test on each row of that matrix (each feature) using the `stattest_table` function, which behaves almost exactly like `stattest`, but takes the matrix as an argument instead of the ballgown object. 

### timecourse experiments
For timecourse experiments, we fit a smooth curve to time using [natural splines](http://en.wikipedia.org/wiki/Spline_interpolation).  The model for expression including these spline terms is compared to a model without any spline terms for the F-test.  The results indicate which features' expression levels change significantly over time. A typical call to `stattest` for a timecourse experiment would look like:
```S
...
You can also use the `timecourse` option to study the effect of continuous covariates (other than time) on expression. For example, in our [paper](http://biorxiv.org/content/early/2014/03/30/003665), we analyzed the effect of RNA quality on expression. Set `timecourse=TRUE` and provide the continuous covariate name as the `covariate` option.

### adjusting for confounders
You can adjust for any or all variables in `pData` when testing for differential expression.  We automatically adjust for library size using the sum of all expression measurements below the 75th percentile for that sample.  If you would like to adjust for other confounders, say sex and age, just provide those confounders as the `adjustvars` argument to `stattest`, e.g., `adjustvars = c('age', 'sex')`.

To disable the automatic library size adjustment, provide `libadjust = FALSE` as an argument to `stattest`. You can provide your own library size adjustment by adding the variable to `pData` and providing that variable name to `adjustvars`. 
