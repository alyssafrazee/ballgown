...
```{r loadcitations, echo=FALSE, message=FALSE, warning=FALSE}
...
* Expression for the features in the transcriptome should be estimated. We provide software called _Tablemaker_ that does this estimation. Tablemaker calls _Cufflinks_ version 2.1.1 `r citep('10.1038/nbt.1621')` to estimate transcript-level FPKMs, and provides several alternative expression measurements (e.g. average per-base coverage, number of mapped reads, etc.) for other features. Expression is estimated for each transcript, exon, and intron (junction) in the assembly. 
...
Ballgown's statistical methods for differential expression testing are straightforward and accurate `r citep('http://biorxiv.org/content/early/2014/03/30/003665')`, but users may wish to use one of the many existing packages for differential expression. Ballgown's data structures make it easy to use table-based packages like limma `r citep(citation('limma'))`, limma Voom `r citep('10.1186/gb-2014-15-2-r29')`, DESeq `r citep('10.1186/gb-2010-11-10-r106')`, DEXSeq `r citep('10.1101/gr.133744.111')`, or EdgeR `r citep('10.1093/bioinformatics/btp616')` for differential expression analysis.  A feature-by-sample expression table can be easily created with a `*expr` function and used directly as input to these or other differential expression packages. 
